...
export const MintTokens: React.FC<Props> = ({ contract, refreshData }) => {
  ...
  const handleMint = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus('success');
      setMessage(`Successfully minted ${amount} TFT to ${recipient.substring(0,6)}...`);
      setRecipient('');
      setAmount('');
      refreshData();
      ...
      console.error(err);
      setStatus('error');
      setMessage(err.shortMessage || err.message || 'Minting failed');
    }
  };
  ...
  return (
    <div className="glass-panel p-6 border-l-4 border-l-secondary">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="ml-auto text-xs bg-secondary/20 text-secondary px-2 py-1 rounded">Owner Only</span>
      </div>
      ...
      <form onSubmit={handleMint} className="space-y-4">
        <div>
          ...
          <input 
            ...
            placeholder="0x..."
            className="input-field focus:ring-secondary focus:border-secondary"
            required
        ...
        <button 
          type="submit" 
          disabled={status === 'pending' || !contract}
          className="btn-primary bg-secondary hover:bg-emerald-600 focus:ring-secondary/50 w-full flex justify-center items-center gap-2 mt-4"
        >
          {status === 'pending' ? (
            <><Loader2 className="animate-spin" size={18} /> Processing...</>
          ) : (
            <><PlusCircle size={18} /> Mint Tokens</>
          )}
        </button>
      </form>
